...
#+AUTHOR: Rahul Sharma, Suraj Kushwah
...
The =doom-themes= are also great. Sometimes =solarized-themes= are great when taking notes.
...
** Cursor
Not very fond of blinking cursors.
#+BEGIN_SRC elisp
  (blink-cursor-mode -1)
#+END_SRC
...
But I can't live without modal cursor when using =evil mode=.
#+BEGIN_SRC elisp
...
* Editing
** Tabs / Spaces
- I use a tab width of 4 which is a sane default as well as the python standard.
  #+BEGIN_SRC elisp
    ;; (setq-default tab-width 4)
  #+END_SRC

- Insert spaces equal to =tab-width= like vim's =set expandtab=. Also make it global
  so that emacs uses spaces instead of tabs.
  #+BEGIN_SRC elisp
...
Sometimes I like to use the nano-theme modeline.
#+BEGIN_SRC elisp
...
** Terminal
- vterm is an advanced terminal for emacs.
#+BEGIN_SRC elisp
  (use-package vterm
    :ensure t)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
      ...
      (defun org-custom-symbols ()
        ;; Keep headings nice indented
        (org-indent-mode)
    ...
    (defun org-custom-headers ()
      ...
      (dolist
        ...
        (set-face-attribute (nth 0 face) nil :font "JetBrains Mono" :weight (nth 3 face) :height (nth 1 face) :foreground (nth 2 face)))
        (set-face-attribute 'org-table nil :font "JetBrains Mono" :weight 'normal :height 1.0 :foreground "#bfafdf"))

      ;; Add the above functions as an orgmode hook
      (add-hook 'org-mode-hook 'org-custom-symbols)
      (add-hook 'org-mode-hook 'org-custom-headers)
#+END_SRC

** Set variables
#+BEGIN_SRC emacs-lisp
  (setq org-directory "~/Documents/org/"
        ; org-agenda-files '("~/Documens/org/agenda.org")
